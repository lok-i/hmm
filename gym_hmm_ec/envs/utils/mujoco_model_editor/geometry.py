...
class Capsule:
    ...
    def render(self,local_origin=np.zeros(3)):
        renderer.render_capsule( local_origin+self.p2,local_origin+self.p1, self.r)

    def pick(self, ray,local_origin=np.zeros(3)):
        return ray.dist2seg(local_origin+self.p1, local_origin+self.p2) <= self.r
